...
public class OsUtils {
    ...
    static public boolean isWeb = Gdx.app.getType() == Application.ApplicationType.WebGL;
    static public boolean isIos = !isAndroid && (!(isWindows || isLinux || isMac || isWeb)) || OS.startsWith("ios");

    public static boolean isMobile = isIos || isAndroid;

    public static final boolean isPC = isWindows || isMac || isLinux || isWeb;
    ...
    static {
        ...
        if (isMOEiOS || (!isAndroid && !isWindows && !isLinux && !isMac && !isWeb)) {
            isIos = true;
            isAndroid = false;
            isWindows = false;
            isLinux = false;
